...
public class BbsMessageDaoImpl extends HibernateBaseDao<BbsMessage, Integer>
	...
	public Pagination getPageByUserId(Integer userId, Integer typeId,Integer pageNo,
			Integer pageSize) {
		String hql = "from BbsMessage bean where bean.receiver.id=:userId and bean.msgType=:typeId order by bean.id desc";
		Finder f = Finder.create(hql);
		f.setParam("userId", userId);
		f.setParam("typeId", typeId);
	...
	public Pagination getPageBySenderUserId(Integer userId, Integer typeId,Integer pageNo,
			Integer pageSize) {
		String hql = "from BbsMessage bean where bean.sender.id=:userId and bean.msgType=:typeId order by bean.id desc";
		Finder f = Finder.create(hql);
		f.setParam("userId", userId);
		f.setParam("typeId", typeId);
		return find(f, pageNo, pageSize);
	...
	public List getListByUserIdStatus(Integer userId, Integer typeId,
			Boolean status) {
		String hql = "from BbsMessage bean where bean.receiver.id=:userId and bean.msgType=:typeId and bean.status=:status order by bean.id desc";
		Finder f = Finder.create(hql);
		f.setParam("userId", userId);
		f.setParam("typeId", typeId);
	...
	public List getListBySenderUserIdStatus(Integer userId, Integer typeId,
			Boolean status) {
		String hql = "from BbsMessage bean where bean.sender.id=:userId and bean.msgType=:typeId and bean.status=:status order by bean.id desc";
		Finder f = Finder.create(hql);
		f.setParam("userId", userId);
		f.setParam("typeId", typeId);
		f.setParam("status", status);
	...
	public int  getTotalCountByReceiverAndTypeIdAndStatus(Integer userId, Integer typeId,Boolean status){
		Finder f = Finder.create("from BbsMessage bean where bean.receiver.id=:userId and bean.msgType=:typeId and bean.status=:status");
		f.setParam("userId", userId);
		f.setParam("typeId", typeId);
		f.setParam("status", status);
	...
	public List<BbsMessage> getListByTypeAndUserId(Integer userId, Integer typeId) {
		String hql = "from BbsMessage bean where bean.user.id=:userId and bean.msgType=:typeId  order by bean.id asc";
		Finder f = Finder.create(hql);
		f.setParam("userId", userId);
		f.setParam("typeId", typeId);
		return find(f);
