...
public class BbsFriendShipDaoImpl extends
	...
	public int  getTotalCountByUserIdAndStatus(Integer userId,Integer status){
		Finder f = Finder.create("from BbsFriendShip bean where bean.friend.id=:userId and bean.status=:status");
	 ...
	 public List<BbsFriendShip>  findAllByUserId(Integer userId){
			String hql = "select bean from BbsFriendShip bean where bean.user.id=? and bean.status=?";
			return find(hql,userId,1);
	 }
