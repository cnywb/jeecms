...
public class BbsTopicMngImpl implements BbsTopicMng {
	...
	public void prime(Integer[] ids, short primeLevel, String reason,
		...
		for (Integer id : ids) {
			...
			bbsOperationMng.saveOpt(topic.getSite(), operator, "精华", reason,
					topic);
			//pointCalculateMng.topicDisgest(topic.getCreater().getId());//添加积分
		}
	}
	...
	public void upTop(Integer[] ids, short topLevel, String reason,
		...
		for (Integer id : ids) {
			...
			bbsOperationMng.saveOpt(topic.getSite(), operator, "置顶", reason,
					topic);
			//pointCalculateMng.topTopic(topic.getCreater().getId());//加积分
		}
	}
	...
	public BbsTopic postTopic(Boolean multipleVote,
			Boolean visibleAfterReply,
			Date voteStartTime,
			Date voteEndTime,Integer userId, Integer siteId, Integer forumId,
		...
		topic.setMultipleVote(multipleVote);
		topic.setVisibleAfterReply(visibleAfterReply);
		topic.setVoteStartTime(voteStartTime);
		topic.setVoteEndTime(voteEndTime);
	...
	public Pagination getForTag(Integer siteId, Integer forumId,Integer parentPostTypeId,Integer postTypeId, Short status,
		...
		Pagination page= dao.getForTag(siteId, forumId,parentPostTypeId,postTypeId, status, primeLevel, keyWords,
				creater, createrId, topLevel, pageNo, pageSize,jinghua);

		List<BbsTopic> list=(List<BbsTopic>) page.getList();
		
		for(BbsTopic t:list){
			List<String> firstPostImageSrcList=bbsPostMng.getFirstPostImageSrcList(t.getId());
			t.setFirstPostImageSrcList(firstPostImageSrcList);//设置主题缩略图
		}
	    page.setList(list);
		return page;
	}
	...
	public BbsTopic deleteById(Integer id) {
		...
		List<BbsPost> postList = bbsPostMng.getPostByTopic(id);//找出主题下所有相关的贴子
		for (BbsPost post : postList) {
			if (post.equals(bean.getForum().getLastPost())) {
				BbsPost post1 = bbsPostMng.getLastPost(bean.getForum().getId(),id);//找出非当前话题下最新的贴子
				if (post1 == null) {
					bean.getForum().setLastPost(null);
					bean.getForum().setLastReply(null);
					bean.getForum().setLastTime(null);
				} else {
					bean.getForum().setLastPost(post1);//设置版块中的最后发贴
					bean.getForum().setLastReply(post1.getCreater());
					bean.getForum().setLastTime(post1.getCreateTime());
					
				}
			}
			bbsPostMng.deleteById(post.getId());//删除回贴
		}
		dao.deleteById(id);//删除话题
		int topicTotal=dao.getCountByForumId(bean.getForum().getId());//重新统计话题数
		int postTotal=bbsPostMng.getCountByForumId(bean.getForum().getId());//重新统计回贴数
		bean.getForum().setTopicTotal(topicTotal);
		bean.getForum().setPostTotal(postTotal);
		return bean;
	}
	...
	public List<BbsTopic> getNewList(Integer count,Integer orderby) {
		// TODO Auto-generated method stub
		return dao.getNewList(count,orderby);
	}
	
	public Pagination getNewList(Integer orderby,int pageNo,int pageSize){
		return dao.getNewList(orderby, pageNo, pageSize);
	}
	...
	public List<BbsTopic> getBeanByForumId(Integer forumId, int count) {
		return dao.getBeanByForumId(forumId, count);
	}
	...
	public void setToHeadlineById(Integer id,Integer forumId){
		dao.setToHeadlineById(id,forumId);
	}
	public List<BbsTopic> getHeadlineByForumId(Integer forumId, int count){
		return dao.getHeadlineByForumId(forumId, count);
	}

	public int  forbidReply(String ids){
		return dao.updateIsForbidReply(ids, true);
	}
	
	public int  allowReply(String ids){
		return dao.updateIsForbidReply(ids, false);
	}
