...
public class BbsUserMngImpl implements BbsUserMng {
	...
	public BbsUser registerMember(String username, String email,String password, String ip, Integer groupId, BbsUserExt userExt,Integer unifUserId){
		...
		user.setGroup(group);
		user.init();
		BbsUser user1 = dao.save(user);
		bbsUserExtMng.save(userExt, user);
		bbsConfigEhCache.setBbsConfigCache(0, 0, 0, 1, user1, 2);
		return user;
	}
	...
	public BbsUser[] deleteByIds(Integer[] ids) {
		cmsUserManager.deleteCmsUserByIds(ids);
	...
	public BbsUser[] deleteBbsUserByIds(Integer[] ids) {
		BbsUser[] beans = new BbsUser[ids.length];
		for (int i = 0, len = ids.length; i < len; i++) {
			beans[i] = deleteBbsUserById(ids[i]);
		}
		return beans;
	}
	...
	@Autowired
	private CmsUserMng cmsUserManager;
