...
public abstract class HibernateSimpleDao {
	...
	@SuppressWarnings("unchecked")
	protected Pagination findBase(FinderBase finder, int pageNo, int pageSize) {
		int totalCount = countQueryResultByFinderBase(finder);
		Pagination p = new Pagination(pageNo, pageSize, totalCount);
		if (totalCount < 1) {
			p.setList(new ArrayList());
			return p;
	 ...
	 * @return
	 */
	protected int countQueryResultByFinderBase(FinderBase finder) {
		Query query = getSession().createQuery(finder.getRowCountHql());
		finder.setParamsToQuery(query);
		if (finder.isCacheable()) {
			query.setCacheable(true);
