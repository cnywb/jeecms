...
public class ImageScale {
	...
	protected ImageScale(File srcFile, File destFile, int zoomWidth,
		...
		if ((width <= zoomWidth && height <= zoomHeight) || srcFile.length() <= 1024 * 1024 * 0.5) {
...
//		}
		...
		try {
			JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(out);
//			encoder.encode(imgBuf);
			JPEGEncodeParam jep = JPEGCodec.getDefaultJPEGEncodeParam(imgBuf);
//			精度0.8差不多可以
			jep.setQuality(1f, true);
			encoder.encode(imgBuf, jep);
		} finally {
			out.close();
		}
	}
	...
	public static void main(String[] args) throws IOException {
		...
		ImageScale.resizeFix(new File("c:/_DSC0574_yuan.JPG"), new File("c:/_DSC0574_yuan_n_n.JPG"),
				700, 600);
