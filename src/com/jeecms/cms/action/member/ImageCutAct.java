...
public class ImageCutAct {
	...
	public String imageCut(String imgSrcPath, Integer imgTop, Integer imgLeft,
		...
		CmsSite site = CmsUtils.getSite(request);
		try {
			doImageCut(imgSrcPath, imgTop, imgLeft, imgWidth, imgHeight,
					reMinWidth, reMinHeight, imgScale, request, site);
			model.addAttribute("uploadNum", uploadNum);
		} catch (Exception e) {
	...
	private void doImageCut(String imgSrcPath, Integer imgTop, Integer imgLeft,
			Integer imgWidth, Integer imgHeight, Integer reMinWidth,
			Integer reMinHeight, Float imgScale, HttpServletRequest request,
			CmsSite site) throws IOException, Exception, FileNotFoundException {
		if (imgWidth > 0) {
	...
	@RequestMapping("/member/imageCutAjax.jspx")
	public void imageCutAjax(String imgSrcPath, Integer imgTop, Integer imgLeft,
			Integer imgWidth, Integer imgHeight, Integer reMinWidth,
			Integer reMinHeight, Float imgScale, Integer uploadNum,
		...
		try {
			...
			imgSrcPath = imgSrcPath.substring(ctx.length());
			File file = fileRepository.retrieve(imgSrcPath);
			AverageImageScale.cutImage(file, file, imgLeft,imgTop, imgWidth, imgHeight);
			object.put("status","1");
