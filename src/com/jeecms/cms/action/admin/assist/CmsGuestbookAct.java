...
public class CmsGuestbookAct {
 ...
 /**
  * ping.qi 2014-07-03 modify
  * add 查询条件
  * 标题关键字，留言时间，留言类别，留言会员，推荐，审核
  * title，createTime，ctg， member，recommend，isChecked
  */
	@RequestMapping("/guestbook/v_list.do")
	public String list(String title,String createTimeMin,String createTimeMax, String member, Integer queryCtgId, String queryRecommend,
			String queryChecked, Integer pageNo, HttpServletRequest request,
			ModelMap model) {
		...
		if(userDepart!=null){
			//ctgIds=getDepartmentCtgIds(userDepart.getId());
		}
		/*Pagination pagination = manager.getPage(site.getId(),ctgIds,null,
				queryRecommend, queryChecked, true, false, cpn(pageNo),
				CookieUtils.getPageSize(request));*/
		Pagination pagination = manager.getPageByKeywordForCms(title, createTimeMin,createTimeMax, member,site.getId(),queryCtgId,ctgIds,null,
				queryRecommend, queryChecked, true, false, cpn(pageNo),
				CookieUtils.getPageSize(request));
	...
	public String update(String title,String createTimeMin,String createTimeMax, String member, Integer queryCtgId, String queryRecommend,
			String queryChecked, String oldreply,CmsGuestbook bean, CmsGuestbookExt ext,
		...
		cmsLogMng.operating(request, "cmsGuestbook.log.update", "id="
				+ bean.getId() + ";title=" + bean.getTitle());
		return list(title,createTimeMin,createTimeMax,member,queryCtgId, queryRecommend, queryChecked, pageNo, request,
				model);
	}

	@RequestMapping("/guestbook/o_delete.do")
	public String delete(String title,String createTimeMin,String createTimeMax, String member, Integer queryCtgId, String queryRecommend,
			String queryChecked, Integer[] ids, Integer pageNo,
			HttpServletRequest request, ModelMap model) {
	...
	public String check(String title,String createTimeMin,String createTimeMax,String member, Integer queryCtgId, String queryRecommend,
			String queryChecked, Integer[] ids, Integer pageNo,
			HttpServletRequest request, ModelMap model) {
		WebErrors errors = validateDelete(ids, request);
		if (errors.hasErrors()) {
	...
	public String cancel_check(String title,String createTimeMin,String createTimeMax,String member, Integer queryCtgId, String queryRecommend,
			String queryChecked, Integer[] ids, Integer pageNo,
			HttpServletRequest request, ModelMap model) {
		WebErrors errors = validateDelete(ids, request);
		if (errors.hasErrors()) {
