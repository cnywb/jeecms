...
public class FrontUtils {
	 ...
	 * bbs用户
	 */
	public static final String BBS_USER="bbsUser";
	/**
	 * 页码
	 */
	...
	/**
	 * BBS链接路径,放在fckeditor.properties配置文件中
	 */
	public static final String BBS_URL = "bbsurl";
	
	/**
	 * 主站点链接路径,放在fckeditor.properties配置文件中
	 */
	...
	public static void frontData(HttpServletRequest request,
		...
		BbsUser bbsuser = CmsUtils.getBbsUser(request);
		String location = RequestUtils.getLocation(request);
		Long startTime = (Long) request.getAttribute(START_TIME);
		frontData(map, site, user, location, startTime,bbsuser);
	}

	public static void frontData(Map<String, Object> map, CmsSite site,
			CmsUser user, String location, Long startTime,BbsUser bbsUser) {
		if (startTime != null) {
		...
		if(bbsUser != null){
			map.put(BBS_USER, bbsUser);
		}
		map.put(BBS_URL, PropertiesLoader.getProperty(BBS_URL));
		map.put(MAIN_URL, PropertiesLoader.getProperty(MAIN_URL));
		map.put(SITE, site);
