...
import com.jeecms.cms.manager.main.CmsWorkflowEventMng;
...
public class CmsUserMngImpl implements CmsUserMng {
	...
	public CmsUser registerMember(String username, String email,
		...
		UnifiedUser unifiedUser = unifiedUserMng.save(username, email,
				password, ip);
		//针对抽奖用户插入信息
		//saveClubUser(unifiedUser, username, email, password);
	...
	public CmsUser registerMember(String username, String email,
		...
		if (unifUserId == null) {
			...
			UnifiedUser unifiedUser = unifiedUserMng.findById(unifUserId);
			user.forMember(unifiedUser);
		}
		CmsGroup group = null;
		if (groupId != null) {
	...
	private void saveClubUser(UnifiedUser unifiedUser,String username, String email,String password){
		//针对抽奖用户插入信息
        try {
        	ClubUser clubUser = new ClubUser();
            clubUser.setUuid(unifiedUser.getId().toString());
            clubUser.setUname(username);
            clubUser.setUemail(email);
			clubUser.setUpass(Encrypt.encryptString(Encrypt.ENCRY_STYLE_MD5,password));
			clubUser.setCreatedate(new Date());
			clubUser.setFlag(1L);
			clubUserMng.save(clubUser);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public CmsUser[] deleteByIds(Integer[] ids) {
		bbsUserManager.deleteBbsUserByIds(ids);
	...
	public CmsUser[] deleteCmsUserByIds(Integer[] ids) {
		CmsUser[] beans = new CmsUser[ids.length];
		for (int i = 0, len = ids.length; i < len; i++) {
			beans[i] = deleteCmsUserById(ids[i]);
		}
		return beans;
	}
	...
	public Pagination getPage(String username, String vin,String createTimeMax,String createTimeMin, 
		int pageNo, int pageSize){
	    return this.dao.getPage(username, vin, createTimeMax,createTimeMin, pageNo, pageSize);
	}
